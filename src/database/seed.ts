...
import type { DeepPartial, EntityManager, EntityTarget, FindOptionsWhere, ObjectLiteral } from "typeorm";
...
async function insertMissing<T extends ObjectLiteral>(manager: EntityManager, target: EntityTarget<T>, key: string, input: ObjectLiteral) {
...
async function seed(manager: EntityManager) {
  ...
  for (const row of sampleData.employees) await insertMissing(manager, Employee, "employeeCode", row);
  ...
  for (const { parentCode, ...row } of sampleData.accounts) {
    const parent = parentCode ? await manager.getRepository(Account).findOneByOrFail({ code: parentCode }) : undefined;
    await insertMissing(manager, Account, "code", { ...row, parent });
  }
  for (const row of sampleData.requests) await insertMissing(manager, ErpRequest, "requestCode", row);
  for (const row of sampleData.payments) {
    const request = await manager.getRepository(ErpRequest).findOneBy({ requestCode: row.requestCode });
    await insertMissing(manager, Payment, "paymentCode", { ...row, request });
  }
  for (const row of sampleData.financialRecords) await insertMissing(manager, FinancialRecord, "recordCode", row);
  for (const row of sampleData.journalEntries) {
    const lines = [];
    for (const line of row.lines) {
      lines.push({ ...line, account: await manager.getRepository(Account).findOneByOrFail({ code: line.accountCode }) });
    }
  ...
  for (const row of sampleData.capital) await insertMissing(manager, CapitalContribution, "reference", row);
  ...
  for (const row of sampleData.recurringExpenses) await insertMissing(manager, RecurringExpense, "code", row);
  await insertMissing(manager, AuditLog, "entity", {
    actor: sampleData.settings.currentUser.name, initials: sampleData.settings.currentUser.initials,
    action: "loaded sample records", entity: "Local development seed", occurredAt: new Date(),
  });
}
...
async function run() {
  try {
    ...
    console.log("Sample data loaded. Existing records were preserved.");
  } finally {
    if (AppDataSource.isInitialized) await AppDataSource.destroy();
  }
}
void run().catch(error => { console.error("Database seed failed:", error); process.exitCode = 1; });
